...
if ( ! class_exists( 'WordPressSettingsFramework' ) ) {
	...
	class WordPressSettingsFramework {
		...
		public function admin_enqueue_scripts() {
			...
			wp_register_style( 'jquery-ui-css', $this->options_url . 'assets/css/jquery-ui.css' );
		...
		public function generate_export_field( $args ) {
			...
			echo '<a target=_blank href="' . esc_attr( $export_url ) . '" class="button" name="' . esc_attr( $args['name'] ) . '" id="' . esc_attr( $args['id'] ) . '">' . esc_attr( $args['value'] ) . '</a>';
		...
		public function generate_group_row_template( $args, $blank = false, $row = 0 ) {
			...
			if ( $args['subfields'] ) {
				...
				foreach ( $args['subfields'] as $subfield ) {
					...
					$class = sprintf( 'wpsf-group__field-wrapper--%s', esc_attr( $subfield['type'] ) );

					$row_template .= sprintf( '<div class="wpsf-group__field-wrapper %s">', esc_attr( $class ) );
					$row_template .= sprintf( '<label for="%s" class="wpsf-group__field-label">%s</label>', esc_attr( $subfield['id'] ), esc_html( $subfield['title'] ) );
		...
		public function generate_textarea_field( $args ) {
			...
			echo '<textarea name="' . esc_attr( $args['name'] ) . '" id="' . esc_attr( $args['id'] ) . '" placeholder="' . esc_attr( $args['placeholder'] ) . '" rows="5" cols="60" class="' . esc_attr( $args['class'] ) . '">' . esc_attr( $args['value'] ) . '</textarea>';
		...
		public function generate_radio_field( $args ) {
			...
			foreach ( $args['choices'] as $value => $text ) {
				...
				echo sprintf( '<label><input type="radio" name="%s" id="%s" value="%s" class="%s" %s> %s</label><br />', esc_attr( $args['name'] ), esc_attr( $field_id ), esc_attr( $value ), esc_attr( $args['class'] ), $checked, esc_html( $text ) );
			}
		...
		public function generate_checkbox_field( $args ) {
			...
			echo '<input type="hidden" name="' . esc_attr( $args['name'] ) . '" value="0" />';
			echo '<label><input type="checkbox" name="' . esc_attr( $args['name'] ) . '" id="' . esc_attr( $args['id'] ) . '" value="1" class="' . esc_attr( $args['class'] ) . '" ' . $checked . '> ' . $args['desc'] . '</label>';
		}
		...
		public function generate_checkboxes_field( $args ) {
			...
			foreach ( $args['choices'] as $value => $text ) {
				...
				echo sprintf( '<li><label><input type="checkbox" name="%s[]" id="%s" value="%s" class="%s" %s> %s</label></li>', esc_attr( $args['name'] ), esc_attr( $field_id ), esc_attr( $value ), esc_attr( $args['class'] ), $checked, esc_html( $text ) );
			}
